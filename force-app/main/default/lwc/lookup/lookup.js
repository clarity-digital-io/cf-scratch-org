...
export default class Lookup extends LightningElement {
  ...
  getkey() {
    console.warn(
      "Lookup.getkey() is deprecated and will be removed in a future version."
    );
  ...
  handleResultClick(event) {
    ...
    // Save selection
    let selectedItem = this._searchResults.filter(
  ...
  dispatchSelectionChange() {
    this.dispatchEvent(
      new CustomEvent("selectionchange", {
        detail: this._curSelection.map((sel) => sel.id)
      })
    );
  }
  ...
  get getDropdownClass() {
    ...
    const isSearchTermValid =
      this._cleanSearchTerm &&
      this._cleanSearchTerm.length >= MINIMAL_SEARCH_TERM_LENGTH;
