...
export default class RecordGroup extends LightningElement {
  ...
  handleSuccess(event) {
    ...
    let values = fields.reduce(
      (accum, field) => {
        let fieldValue = {};
        accum[field] = event.detail.fields.hasOwnProperty(field)
          ? event.detail.fields[field].displayValue
          : null;
        console.log(
          fieldValue,
          event.detail.fields.hasOwnProperty(field),
          field
        );
        return accum;
      },
